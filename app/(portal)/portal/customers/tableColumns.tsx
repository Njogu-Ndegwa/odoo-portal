'use client'

import { useTranslations } from 'next-intl'
import { TableColumn } from '@/components/table/table'

export function usePortalCustomerColumns(): TableColumn<any>[] {
    const t = useTranslations('portal.customers.columns')
    const tt = useTranslations('portal.customers.types')

    return [
        {
            header: t('name'),
            accessor: 'name' as keyof any,
            cellRenderer: (value: unknown, item: any) => {
                const name = item.name || '-';
                const capitalizedName = name.charAt(0).toUpperCase() + name.slice(1);
                return (
                    <div className="font-medium text-gray-800 dark:text-gray-100">
                        {capitalizedName}
                    </div>
                );
            }
        },
        {
            header: t('email'),
            accessor: 'email' as keyof any,
            cellRenderer: (value: unknown, item: any) => (
                <div className="max-w-md truncate">
                    {item.email || '-'}
                </div>
            )
        },
        {
            header: t('phone'),
            accessor: 'phone' as keyof any,
            cellRenderer: (value: unknown, item: any) => (
                <div className="max-w-md truncate">
                    {item.phone || '-'}
                </div>
            )
        },
        {
            header: t('city'),
            accessor: 'city' as keyof any,
            cellRenderer: (value: unknown, item: any) => (
                <div className="max-w-md truncate">
                    {item.city || '-'}
                </div>
            )
        },
        {
            header: t('street'),
            accessor: 'street' as keyof any,
            cellRenderer: (value: unknown, item: any) => (
                <div className="max-w-md truncate">
                    {item.street || '-'}
                </div>
            )
        },
        {
            header: t('type'),
            accessor: 'isCompany' as keyof any,
            cellRenderer: (value: unknown, item: any) => {
                const isCompany = item.isCompany;
                const typeColor = isCompany
                    ? "bg-blue-100 text-blue-800"
                    : "bg-green-100 text-green-800";
                return (
                    <div className={`px-2 py-1 rounded-full text-xs font-medium inline-block ${typeColor}`}>
                        {isCompany ? tt('company') : tt('individual')}
                    </div>
                );
            }
        },
        {
            header: t('createdAt'),
            accessor: 'createdAt' as keyof any,
            cellRenderer: (value: unknown, item: any) => {
                try {
                    if (!item.createdAt) return <div>-</div>;
                    const d = new Date(item.createdAt);
                    return (
                        <div className="max-w-md truncate">
                            {d.toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' })}
                        </div>
                    );
                } catch {
                    return <div>-</div>;
                }
            }
        },
    ]
}

export const dropdownOptions = [
    {
        id: 0,
        value: 'Assign to Agent'
    },
]
